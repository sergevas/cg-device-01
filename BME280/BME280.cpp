...
TrimmParams BME280::readTrimmParams()
{
...
int32_t BME280::getFineTemp(RawData rd, TrimmParams tp)
{
...
int32_t BME280::compensateTemp(int32_t fineTemp)
{
...
uint32_t BME280::compensatePressure(RawData rd, TrimmParams tp, int32_t fineTemp)
{
...
uint32_t BME280::compensateHumidity(RawData rd, TrimmParams tp, int32_t fineTemp)
{
  int32_t v_x1_u32r;
  v_x1_u32r = (fineTemp - ((int32_t)76800));
  v_x1_u32r = (((((rd.hum << 14) - (((int32_t)tp.digH4) << 20) - (((int32_t)tp.digH5) * v_x1_u32r)) + ((int32_t)16384)) >> 15) * (((((((v_x1_u32r * ((int32_t)tp.digH6)) >> 10) * (((v_x1_u32r * ((int32_t)tp.digH3)) >> 11) + ((int32_t)32768))) >> 10) + ((int32_t)2097152)) * ((int32_t)tp.digH2) + 8192) >> 14));
...
double BME280::getTemperature(int32_t inTemp)
{
  return (double)inTemp / 100.0;
}

double BME280::getPressure(uint32_t inPres)
{
  return (double)inPres / 256.0;
}

double BME280::getHumidity(uint32_t inHum)
{
  return (double)inHum / 1024.0;
}

void BME280::readAll(double *temperature, double *pressure, double *humidity)
{
  RawData rd;
  int32_t fineTemp;
  rd = burstRead();
